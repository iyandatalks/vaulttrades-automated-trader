...
export interface Signal{id:string;symbol:string;strategy:string;side:Side;timeframe:Timeframe;entry:number;stopLoss:number;takeProfit:number;tp1?:number;tp2?:number;tp3?:number;confidence:number;status:SignalStatus;createdAt:string;source:'AUTOMATION';reason:string[];higherTimeframeConfirmed:boolean;quality?:string;}
